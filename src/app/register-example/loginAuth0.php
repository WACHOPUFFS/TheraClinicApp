...
if (isset($_POST['sub'])) {
    ...
    if ($result) {
        ...
        if ($result->num_rows == 1) {
            ...
            $userId = $user['id'];
            $state = $user['state'];
            $username = $user['username']; // Asume que 'username' está en la tabla 'users'
                    ...
                    if ($state == 'active') {
                        ...
                        echo json_encode(
                            array(
                                ...
                                "message" => "Inicio de sesión exitoso.",
                                "userId" => $userId, // ID de usuario incluido en la respuesta
                                "username" => $username,   // Nombre de usuario incluido en la respuesta
                                "principalCompanies" => $principalCompanies,
                            )
                        );
                    } else {
